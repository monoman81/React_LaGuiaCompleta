...
type HeaderProps = {
    ...
    updateQty: (id: Guitar["id"], increment: number) => void
    clearCart: () => void
    isEmpty: boolean
    cartTotal: number
}

export default function Header({ cart, removeFromCart, updateQty, clearCart, isEmpty, cartTotal } : HeaderProps) {
                                            ...
                                            {cart.map(item => (
                                                <tr key={item.id}>
                                                    ...
                                                    <td className="flex align-items-start gap-4">
                                                        <button type="button" className="btn btn-dark" onClick={() => updateQty(item.id, -1)}>
                                                            -
                                                        </button>
                                                        {item.quantity}
                                                        <button type="button" className="btn btn-dark" onClick={() => updateQty(item.id, 1)}>
